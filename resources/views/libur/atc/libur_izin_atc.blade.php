    ...
    var dtTable = $('#myTable').DataTable({
        ...
        columns: [
            ...
            { data: 'bukti', name: null, orderable: false, searchable:false,
                render: function(data, type, row, meta){
                    if(data){
                        let link = '{{ asset("storage/bukti") }}/'+data;
                        return `<a href="${link}">Lihat</a>`;
                    }
                    return '-';
                }
            },
            ...
            { data: 'from', name: 'from', orderable: false, searchable:false,
                render: function (data, type, row, meta) {
                    return moment(row.from).locale('id').format('DD MMMM YYYY HH:mm');
                }
            },
            { data: 'to', name: 'to', orderable: false, searchable:false,
                render: function (data, type, row, meta) {
                    return moment(row.to).locale('id').format('DD MMMM YYYY HH:mm');
                }
            },
