                ...
                <div class="col-12 col-sm text-left text-sm-right mt-2 mt-sm-0">
                    @if (in_array($item->status, [
                        ...
                        <button type="button" class="btn btn-success"
                            ...
                            data-json='@json(["msg" => "Apakah anda yakin menyetujui izin keluar " . ($item->data_employees?->name ?: "-") . "?", "id" => $item->id])'
                        ...
                        <button type="button" class="btn btn-danger"
                            data-toggle="modal" data-target="#modalConfirmDelete"
                            data-json='@json(["msg" => "Apakah anda yakin menolak izin keluar " . ($item->data_employees?->name ?: "-") . "?", "id" => $item->id])'
                            data-dispatch="wireReject()"
                            data-submit-label="Tolak">
                            <i class="fas fa-ban fa-fw"></i> Tolak
                        </button>
                    @endif
                    <a href="{{ route('rdp.persetujuan.izin-keluar.index') }}" class="btn btn-secondary">
