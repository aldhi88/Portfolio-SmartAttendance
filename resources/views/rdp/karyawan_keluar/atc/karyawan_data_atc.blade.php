...
<script>
    ...
    var dtTable = $('#myTable').DataTable({
        ...
        order: [[1, 'desc']],
        columnDefs: [
            { className: 'text-left', targets: [2,5,6,7,8] },
            { className: 'px-0', targets: [0] },
            { className: 'text-center', targets: ['_all'] },
        ],
        ajax: '{{ route("rdp.pengajuan.izin-keluar.indexDT") }}',
        columns: [
            {
                ...
                render: function(data) {
                    ...
                    const dtJson = {
                        msg: `Apakah anda yakin membatalkan pengajuan ${data.nomor_sk_keluar || '-'}?`,
                        id: data.id
                    };
                    return `
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu">
                                <a href="{{ url('rdp/pengajuan/izin-keluar/detail') }}/${data.id}" class="dropdown-item">
                                    <i class="fas fa-eye fa-fw"></i> Detail Data
                                </a>
                                ${canEdit ? `<a href="{{ url('rdp/pengajuan/izin-keluar/edit') }}/${data.id}" class="dropdown-item">
                                    <i class="fas fa-edit fa-fw"></i> Edit Data
                                </a>` : ''}
                                ${canPendataanAset ? `<a href="{{ url('rdp/pengajuan/izin-keluar/pendataan-aset') }}/${data.id}" class="dropdown-item">
            ...
            { data: 'nomor_sk_keluar', name: 'nomor_sk_keluar', orderable: true, searchable:true },
            {
                data: 'tanggal_sk_keluar', name: 'tanggal_sk_keluar', orderable: true, searchable:true,
