...
class RdpKaryawanKeluarRepo
{
    ...
    public const PIMPINAN_APPROVED_STATUS = 'Berkas Disetujui Pimpinan, menuggu pendataan aset keluar';
    public const ASSET_SUBMITTED_STATUS = 'Pengajuan Pendataan Aset Keluar';
    public const ASSET_SPV_APPROVED_STATUS = 'Pendataan Disetujui SPV, menuggu Pimpinan';
    public const FINISHED_STATUS = 'Keluar RDP Selesai';
    public const CANCEL_STATUS = 'Keluar RDP Dibatalkan';
    ...
    public const FILE_DIR = 'rdp/izin-keluar';

    public static function getByKey($id)
    {
        return RdpKaryawanKeluar::with([
    ...
    public static function getCurrentRumahByEmployee($employeeId)
    {
        if (empty($employeeId)) {
            return null;
        }
        ...
        return RdpKaryawanMasuk::query()
            ->with('rdp_master_rumahs.rdp_master_clusters:id,nama_cluster')
    ...
    public static function create($data)
    {
        try {
            DB::transaction(function () use ($data) {
                ...
                if (!self::employeeOccupiesRumah($payload['data_employee_id'] ?? null, $payload['rdp_master_rumah_id'] ?? null)) {
                    throw new \Exception('Karyawan tidak sedang menempati rumah tersebut.');
                }

                if (self::hasActiveOrPendingKeluar($payload['data_employee_id'] ?? null, $payload['rdp_master_rumah_id'] ?? null)) {
                    throw new \Exception('Karyawan masih memiliki pengajuan keluar aktif.');
                }

                RdpKaryawanKeluar::create($payload);
            });

            return true;
        } catch (\Exception $e) {
            Log::error("Insert rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function createWithAsets($data, $asetData)
    {
        try {
            ...
            Log::error("Insert rdp_karyawan_keluars with aset failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function update($id, $data, $allowForwardStatus = true)
    {
        try {
            DB::transaction(function () use ($id, $data, $allowForwardStatus) {
                ...
                if (!self::employeeOccupiesRumah($payload['data_employee_id'] ?? null, $payload['rdp_master_rumah_id'] ?? null, $item->id)) {
                    throw new \Exception('Karyawan tidak sedang menempati rumah tersebut.');
                }

                if (self::hasActiveOrPendingKeluar($payload['data_employee_id'] ?? null, $payload['rdp_master_rumah_id'] ?? null, $item->id)) {
                    throw new \Exception('Karyawan masih memiliki pengajuan keluar aktif.');
                }
            ...
            Log::error("Update rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function cancel($id)
    {
        try {
            ...
            Log::error("Cancel rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function approveBerkasAdmin($id, $rumahId)
    {
        try {
            ...
            if (!self::employeeOccupiesRumah($item->data_employee_id, $rumahId, $item->id, false)) {
                return false;
            }
            ...
            Log::error("Approve berkas rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function requestRevisionBerkasAdmin($id, $catatan)
    {
        try {
            ...
            Log::error("Request revision berkas rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function cancelByAdmin($id)
    {
        try {
            ...
            Log::error("Cancel by admin rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
        ...
        try {
            ...
            if ($item->status !== self::SPV_APPROVED_STATUS) {
                ...
                DB::transaction(function () use ($item) {
                    if (!self::employeeOccupiesRumah($item->data_employee_id, $item->rdp_master_rumah_id, $item->id, false)) {
                        throw new \Exception('Karyawan tidak sedang menempati rumah tersebut.');
                    }
            ...
            Log::error("Approve pimpinan rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function rejectPimpinan($id)
    {
        try {
            ...
            Log::error("Reject pimpinan rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function submitPendataanAsets($id, $asetData, $nextStatus = self::ASSET_SUBMITTED_STATUS)
    {
        try {
            ...
            Log::error("Submit pendataan aset rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public static function approvePendataanAset($id)
    {
        try {
            ...
            Log::error("Approve pendataan aset rdp_karyawan_keluars failed", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    protected static function normalizePayload($data, $existing = null)
    {
        ...
        if (!empty($data['file_sk_keluar'])) {
            $payload['file_sk_keluar'] = self::storeFile($data['file_sk_keluar'], $existing?->file_sk_keluar);
        }
    ...
    public static function hasActiveOrPendingKeluar($employeeId, $rumahId = null, $excludeId = null)
    {
        if (empty($employeeId)) {
            return false;
        }
        ...
        return RdpKaryawanKeluar::query()
            ->where('data_employee_id', $employeeId)
            ->when($rumahId, fn ($query) => $query->where('rdp_master_rumah_id', $rumahId))
            ->when($excludeId, fn ($query) => $query->whereKeyNot($excludeId))
            ->whereNotIn('status', [self::FINISHED_STATUS, self::CANCEL_STATUS])
            ->exists();
    }

    public static function employeeOccupiesRumah($employeeId, $rumahId, $currentKeluarId = null, $allowCurrentRecord = true)
    {
        if (empty($employeeId) || empty($rumahId)) {
            return false;
        }
        ...
        if (
            $allowCurrentRecord
            &&
            !empty($currentKeluarId)
            && RdpKaryawanKeluar::whereKey($currentKeluarId)
                ->where('data_employee_id', $employeeId)
                ->where('rdp_master_rumah_id', $rumahId)
                ->exists()
        ) {
        ...
        $rumah = self::getCurrentRumahByEmployee($employeeId);

        return (int) ($rumah?->id) === (int) $rumahId;
    }
    ...
    {
        $fileName = uniqid('sk_keluar_', true) . '.' . $file->extension();
