...
class AdminCreate extends Component
{
    ...
    public $form = [
        'status' => RdpPermintaanRepo::FINISHED_STATUS,
    ];
    public $items = [];
    ...
    {
        $this->dt['penempatans'] = RdpPermintaanRepo::getActivePenempatans()->toArray();
        $this->addItem();
    }
    ...
    {
        return collect($this->dt['penempatans'])
            ->firstWhere('id', (int) ($this->form['rdp_karyawan_masuk_id'] ?? 0));
    }
    ...
    public function wireSubmit()
    {
        ...
        $form['form']['status'] = RdpPermintaanRepo::FINISHED_STATUS;

        if (RdpPermintaanRepo::create($form['form'], $form['items'])) {
            session()->flash('success', 'Permintaan berhasil ditambahkan dan langsung diselesaikan.');
            return redirect()->route('rdp.permintaan.index');
        }
    ...
    public $validationAttributes = [
        'form.rdp_karyawan_masuk_id' => 'Karyawan/Rumah',
    ...
    {
        return view('rdp.permintaan.admin_create');
    }
}
